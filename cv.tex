...
{
	Driving AI-native strategy and architecture modernization across teams, with a focus on unblocking critical delivery paths and improving platform scalability. Increased throughput by 10x and reduced latency in a primary system bottleneck, then improved search quality and performance to deliver measurable customer impact. Reworked a problematic core codebase to enable a full AI-native coding pipeline where a single running agent supports developer code, test, and deploy workflows end to end. Served as a cross-team architectural liaison while leading exploratory work on knowledge-graph-augmented AI systems and fine-grained, row-level authorization at scale using SpiceDB.
}
\entry
{2020--2025}
...
{
	Co-founded and built a startup focused on unifying streaming and relational data into a single product platform. Served as the core developer and led a team of engineers from early market exploration to product-market fit, partnering directly with customers to build integrations and production deployments. Scaled the business to over \$1M in annual revenue while remaining fully bootstrapped. Extended the platform with modern AI capabilities including neuro-symbolic database exploration, MCP servers, data lakes, RAG workflows, vector support, and customer-specific data repositories.
}
\entry
{2018--2020}
